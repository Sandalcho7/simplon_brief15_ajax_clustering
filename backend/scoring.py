...
def get_kmeans_silhouette_score():
    kmeans_labels = kmeans_model.predict(formatted_data[["Spending Score (1-100)", "Annual Income (k$)"]].values)

    return silhouette_score(X, kmeans_labels)


def get_dbscan_silhouette_score():
    dbscan_labels = dbscan_model.fit_predict(formatted_data[["Spending Score (1-100)", "Annual Income (k$)"]].values)
